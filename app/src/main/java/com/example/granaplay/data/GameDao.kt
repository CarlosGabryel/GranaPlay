...
interface GameDao {
    ...
    // NOVO: Adicionado para corrigir o erro no AuthViewModel
    ...
    @Query("SELECT COUNT(*) FROM usuario_licoes INNER JOIN licoes ON usuario_licoes.licao_id = licoes.id WHERE usuario_licoes.usuario_id = :usuarioId AND licoes.modulo_id = :moduloId")
    suspend fun contarLicoesConcluidasNoModulo(usuarioId: Long, moduloId: Long): Int

    @Query("SELECT * FROM licoes WHERE modulo_id = :moduloId AND id NOT IN (SELECT licao_id FROM usuario_licoes WHERE usuario_id = :usuarioId) ORDER BY ordem ASC LIMIT 1")
    suspend fun getProximaLicaoPendente(usuarioId: Long, moduloId: Long): Licao?

    // NOVO: Adicionado para corrigir o erro no ProfileViewModel
    @Query("SELECT COUNT(*) FROM usuario_licoes WHERE usuario_id = :userId")
    fun contarLicoesConcluidasPeloUsuario(userId: Long): Flow<Int>
}